...
def create_visualization(
    ...
    for med_type in MED_TYPES:
        ...
        if col_name in df.columns:
            med_matrices[med_type] = df.pivot(
                index="minute_of_day", columns="day", values=col_name
            ).values
        else:
    ...
    total_cols = (
        (num_days * day_width)
        + ((num_days - 1) * day_padding)
        + (num_separators * (separator_width - day_padding))
    )
    ...
    ax.imshow(
        ...
        extent=[left_margin, total_cols + left_margin, rows, 0],
    )

    # Month labels
    for label_col, label in section_info:
        ax.text(
            label_col + left_margin,
            -8,
            label,
            ha="left",
            va="bottom",
    ...
    for i, (color, label) in enumerate(legend_items):
        ...
        ax.add_patch(
            plt.Rectangle(
                (legend_x, y_pos - 4), square_size, square_size, facecolor=color, edgecolor="none"
            )
        )
        ax.text(
            legend_x + square_size + 3,
            y_pos,
            label,
            ha="left",
            va="center",
